...
import torch
import torch.nn as nn
from kd_losses.pkt import PKTCosSim
...
# Create an instance of the PKTCosSim loss function
pkt_loss = PKTCosSim()
...
for epoch in range(num_epochs):
    for batch_idx, (data, target) in enumerate(train_loader):
        # Forward pass
        output = model(data)
        # Calculate the PKT loss
        pkt_loss_value = pkt_loss(output_s, output_t)
        # Calculate the total loss
        total_loss = pkt_loss_value + other_loss_terms
        # Backward pass
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
